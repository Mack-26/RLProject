...
class AdaptiveAlphaNetwork(nn.Module):
    def __init__(self):
        super().__init__()
        self.w_global = nn.Parameter(torch.randn(1) * 0.01)
        self.w_local = nn.Parameter(torch.randn(1) * 0.01)

        self.w_disagree = nn.Parameter(torch.randn(1) * 0.01)
        self.w_team_var = nn.Parameter(torch.randn(1) * 0.01)
        self.w_risk = nn.Parameter(torch.randn(1) * 0.01)
        self.w_ratio = nn.Parameter(torch.randn(1) * 0.01)
    ...
    def forward(self, v_global: torch.Tensor, v_local: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        f_disagree = torch.abs(v_global - v_local)
        ...
        f_risk = torch.minimum(v_global, v_local)
        
        f_ratio = v_global / (torch.abs(v_local) + 1e-5)
        
        logits = (self.w_global * v_global) + \
                 (self.w_local * v_local) + \
                 (self.w_disagree * f_disagree) + \
                 (self.w_team_var * f_team_var) + \
                 (self.w_risk * f_risk) + \
                 (self.w_ratio * f_ratio) + \
                 self.phi
                 
        alpha = torch.sigmoid(logits)
        self.last_alpha_mean = alpha.mean().detach().cpu()
...
class AdaptiveMappo(Algorithm):
    ...
    def get_critic(self, group: str) -> TensorDictModule:
        ...
        if self.share_param_critic:
            ...
            # local_critic_output_spec = Composite({"state_value_local": Unbounded(shape=(1,))})
        else:
            global_critic_output_spec = Composite(
                {
                    group: Composite(
                        {"state_value_global": Unbounded(shape=(n_agents, 1))},
                        shape=(n_agents,),
                    )
                }
            )
        ...
        local_critic_output_spec = Composite(
            {
                group: Composite(
                    {"state_value_local": Unbounded(shape=(n_agents, 1))},
                    shape=(n_agents,),
                )
            }
        )
        ...
        if self.state_spec is not None:
            ...
            global_critic_input_spec = Composite(
                {group: self.observation_spec[group].clone().to(self.device)}
            )

        local_critic_input_spec = Composite(
            {group: self.observation_spec[group].clone().to(self.device)}
        )
        ...
        adaptive_net = AdaptiveAlphaNetwork().to(self.device)
...
class AdaptiveMappoConfig(AlgorithmConfig):
    """Configuration dataclass for :class:`~benchmarl.algorithms.AdaptiveMappo`."""
